...
function [prefOri, prefSI, prefOriPres, average_responses, preforiIdx] = calcOSI(stimData, ustm, stimAngle, FLAG_OSI, useCVmethod)
...
if iscell(ustm)
    ...
    isMoving = true(size(stimAngle)); % if no ustm input given, assume all columns are to be used
end
...
for iAngle=1:numel(unique_stimAngle)
    ...
    rgt{2} = ic==iAngle;
    % Take the median over all occurances of this angle and put it in the holding array
    average_responses(lft{:}) = nanmedian(stimData(rgt{:}),2);
end
...
%    debugging: average_responses = [0,0,0.4,0];
    ...
    if FLAG_OSI
        prefOri = prefOri / 2;
        unique_stimAngle = unique_stimAngle / 2;
    end
    ...
    prefSI = abs(osi);
    presOris = repmat(unique_stimAngle,size(prefOri,1),1);
    [~, preforiIdx] = min(abs(repmat(prefOri,1,size(unique_stimAngle,2))-presOris),[],2);
    prefOriPres = nan(size(prefOri,1),1);
    prefOriMag = nan(size(prefOri,1),1);
    for iROI = 1:size(prefOri,1)
        prefOriPres(iROI,1) = presOris(iROI,preforiIdx(iROI));  % FIX... 178 = 135 right now
        prefOriMag(iROI,1) = average_responses(iROI,preforiIdx(iROI)); 
    end
